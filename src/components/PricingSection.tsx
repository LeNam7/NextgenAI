      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          {pricingPackages.map((pkg) => (
            <div
              key={pkg.name}
              className={`rounded-2xl border p-8 flex flex-col justify-between transition-all duration-300 relative bg-white ${
                pkg.isPopular
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-655 mt-2 leading-relaxed">{pkg.tagline}</p>
                </div>
                ...
                <ul className="space-y-3">
                  {pkg.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-2.5 text-xs text-slate-700">
                      <Check className="w-4 h-4 text-blue-650 flex-shrink-0 mt-0.5" />
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
              ...
              <div className="pt-8 mt-8 border-t border-slate-100">
                <a
                  href="#lien-he"
                  className={`w-full inline-flex items-center justify-center px-4 py-3 rounded-xl text-xs font-bold transition-all active:scale-98 cursor-pointer ${
                    pkg.isPopular
                      ? "bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-500/10"
                      : "bg-slate-50 border border-slate-200 hover:border-slate-350 text-slate-705 hover:text-slate-900"
                  }`}
                >
                  {pkg.ctaText}
                </a>
